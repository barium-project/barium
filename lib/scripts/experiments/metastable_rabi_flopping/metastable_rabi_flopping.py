...
class metastable_rabi_flopping(experiment):
    ...
    exp_parameters = [('MetastableRabiFlopping','Sequences_Per_Point'),
                      ('MetastableRabiFlopping','Start_Time'),
                      ('MetastableRabiFlopping','Stop_Time'),
                      ('MetastableRabiFlopping','Time_Step'),
                      ('MetastableRabiFlopping', 'Frequency_Start'),
                      ('MetastableRabiFlopping', 'Frequency_Stop'),
                      ('MetastableRabiFlopping', 'Frequency_Step'),
                      ('MetastableRabiFlopping','dc_threshold'),
                      ('MetastableRabiFlopping','Mode'),
                      ('MetastableRabiFlopping','Scan'),
                      ]
    ...
    def initialize(self, cxn, context, ident):
        self.ident = ident
        self.cxn = labrad.connect(name = 'Metastable Rabi Flopping')
        self.cxnwlm = labrad.connect(multiplexer_config.ip, name = 'Metastable Rabi Flopping', password = 'lab')
        ...
        self.cycles = self.p.MetastableRabiFlopping.Sequences_Per_Point
        self.start_time = self.p.MetastableRabiFlopping.Start_Time
        self.stop_time = self.p.MetastableRabiFlopping.Stop_Time
        self.step_time = self.p.MetastableRabiFlopping.Time_Step
        self.start_freq = self.p.MetastableRabiFlopping.Frequency_Start
        self.stop_freq = self.p.MetastableRabiFlopping.Frequency_Stop
        self.step_freq = self.p.MetastableRabiFlopping.Frequency_Step
        self.disc = self.pv.get_parameter('StateReadout','state_readout_threshold')
        self.dc_thresh = self.p.MetastableRabiFlopping.dc_threshold
        self.mode = self.p.MetastableRabiFlopping.Mode
        self.scan = self.p.MetastableRabiFlopping.Scan
        self.TTL_614 = self.p.MetastableStatePreparation133.TTL_614
    ...
    def run(self, cxn, context):
        ...
        if self.scan == 'time':
            ...
            self.pulser.switch_auto(self.TTL_614,False)

            for i in range(len(t)):
                if self.pause_or_stop():
                    # Turn on 614 if aborting experiment
                    self.pulser.switch_manual(self.TTL_614,True)
                    return
        ...
        if self.scan == 'frequency':
            ...
            freq = np.linspace(self.start_freq['MHz'],self.stop_freq['MHz'],\
                    np.round(abs(self.stop_freq['MHz']-self.start_freq['MHz'])/self.step_freq['MHz'],0) +1)
            ...
            for i in range(len(freq)):
                ...
                self.p.MetaStableRaman.frequency_780_2 = WithUnit(freq[i],'MHz')
                self.program_pulse_sequence()
                ...
                while True:
                    ...
                    self.dv.add(2*freq[i] , fid, context = self.c_prob)
                    # Save histogram
                    # We want to save all the experimental data, include dc as sd counts
                    exp_list = np.arange(self.cycles)
                    data = np.column_stack((exp_list, dc_counts, sd_counts))
    ...
    def set_up_datavault(self):
        ...
        # prob
        dataset = self.dv.new('Metastable_Rabi_prob',[('run', 'time')], [('num', 'prob', 'num')], context = self.c_prob)
        # add dv params
        ...
        #dataset1 = self.dv.new('Metastable_Rabi_Time_Tags',[('run', 'arb u')], [('Time_Tags', 'Time', 's')], context = self.c_time_tags)
