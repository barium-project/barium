...
from barium.lib.scripts.pulse_sequences.GreenMicrowaveSweep import green_microwave_sweep as main_sequence
...
class green_microwave_sweep(experiment):

    name = 'Green Microwave Sweep'
    ...
    def initialize(self, cxn, context, ident):
        self.ident = ident
        self.cxn = labrad.connect(name = 'Green Microwave Sweep')
        self.cxnwlm = labrad.connect(multiplexer_config.ip, name = 'Green Microwave Sweep', password = 'lab')
        ...
        if self.m_sequence == 'single':
            self.LO_freq = self.p.GreenMicrowaves133.LO_frequency
    ...
    def run(self, cxn, context):
        ...
        for i in range(len(freq)):
            ...
            if self.m_sequence == 'single':
                self.p.GreenMicrowaves133.frequency_microwaves = WithUnit(freq[i],'MHz')
            elif self.m_sequence == 'composite_1':
