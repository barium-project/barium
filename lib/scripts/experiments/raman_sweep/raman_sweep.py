...
from barium.lib.scripts.pulse_sequences.RamanSweep133 import raman_sweep as main_sequence
...
class raman_sweep(experiment):

    name = 'Raman Sweep'

    exp_parameters = [
                      ('RamanSweep133', 'Sequences_Per_Point'),
                      ('RamanSweep133', 'Start_Frequency'),
                      ('RamanSweep133', 'Stop_Frequency'),
                      ('RamanSweep133', 'Frequency_Step'),
                      ('RamanSweep133', 'dc_threshold'),
                      ('RamanSweep133', 'Windfreak_Channel'),
                      ]
    ...
    def initialize(self, cxn, context, ident):
        self.ident = ident
        self.cxn = labrad.connect(name = 'Raman Sweep')
        self.cxnwlm = labrad.connect(multiplexer_config.ip, name = 'Raman Sweep', password = 'lab')
        ...
        self.wind = self.cxn.windfreak_server
        ...
        self.cycles = self.p.RamanSweep133.Sequences_Per_Point
        self.start_frequency = self.p.RamanSweep133.Start_Frequency
        self.stop_frequency = self.p.RamanSweep133.Stop_Frequency
        self.step_frequency = self.p.RamanSweep133.Frequency_Step
        self.state_detection = self.p.RamanSweep133.State_Detection
        self.dc_thresh = self.p.RamanSweep133.dc_threshold
        self.channel= self.p.RamanSweep133.Windfreak_Channel
        self.disc = self.pv.get_parameter('StateReadout','state_readout_threshold')
##        self.m_sequence = self.p.RamanSweep133.microwave_pulse_sequence
...
##        self.HPA.select_device(self.device_mapA['GPIB0::19'])
    ...
    def run(self, cxn, context):
        ...
        #choose channel A or B on windfreak
        if self.channel == "A":
            self.wind.set_channel(0)
        elif self.channel == "B":
            self.wind.set_channel(1)
        ...
        for i in range(len(freq)):
            ...
            # Set the windfreak frequency
            self.wind.set_freq(freq[i])
            #start pulse sequence (raman133.py)
    ...
    def set_up_datavault(self):
        ...
        dataset = self.dv.new('RamanSweep_prob',[('freq', 'MHz')], [('Probability', 'Probability', 'num')], context = self.c_prob)
        # add dv params
        ...
        dataset1 = self.dv.new('RamanSweep_hist',[('freq', 'MHz')], [('Counts', 'Counts', 'num')], context = self.c_hist)
        # add dv params
        for parameter in self.p:
        ...
        dataset2 = self.dv.new('RamanSweep_dc_hist',[('run', 'arb u')],\
    ...
    def set_start_frequency(self):
        self.wind.set_freq(self.start_frequency["MHz"])
