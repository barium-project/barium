    ...
    def sequence(self):
       ...
       # use this to turn the DDS to a very low power right before we turn it on
        ...
        if p.shelving_duration != 0:
            ...
            self.addTTL(p.TTL_650, self.start + switch_on_delay, p.shelving_duration)
            self.addTTL(p.TTL_493, self.start + switch_on_delay, p.shelving_duration)
            # If we want the 493 on turn on the 493 DDS rf switch
            if p.amplitude_493['dBm'] != -48.0:
                self.addTTL(p.TTL_493_DDS, self.start + switch_on_delay, p.shelving_duration)
