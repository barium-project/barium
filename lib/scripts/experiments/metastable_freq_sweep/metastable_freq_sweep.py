...
from barium.lib.scripts.pulse_sequences.MetastableFreqSweep import metastable_freq_sweep as main_sequence
...
class metastable_freq_sweep(experiment):

    name = 'Metastable Freq Sweep'

    exp_parameters = [('MetastableFreqSweepExp','Sequences_Per_Point'),
                      ('MetastableFreqSweepExp', 'Start_Frequency'),
                      ('MetastableFreqSweepExp', 'Stop_Frequency'),
                      ('MetastableFreqSweepExp', 'Frequency_Step'),
                      ('MetastableFreqSweepExp','dc_threshold'),
                      ('MetastableFreqSweepExp', 'E2_Amplitude'),
                      ('MetastableFreqSweepExp', 'E2_Frequency'),
                      ('MetastableFreqSweepExp', 'Mode'),
                      ]
    ...
    def initialize(self, cxn, context, ident):
        self.ident = ident
        self.cxn = labrad.connect(name ='Metastable Freq')
        self.cxnwlm = labrad.connect(multiplexer_config.ip, name = 'Metastable Freq', password = 'lab')
        ...
        self.cycles = self.p.MetastableFreqSweepExp.Sequences_Per_Point
        self.start_frequency = self.p.MetastableFreqSweepExp.Start_Frequency
        self.stop_frequency = self.p.MetastableFreqSweepExp.Stop_Frequency
        self.step_frequency = self.p.MetastableFreqSweepExp.Frequency_Step
        self.dc_thresh = self.p.MetastableFreqSweepExp.dc_threshold
        self.hp_freq = self.p.MetastableFreqSweepExp.E2_Frequency
        self.hp_amp = self.p.MetastableFreqSweepExp.E2_Amplitude
        self.disc = self.pv.get_parameter('StateReadout','state_readout_threshold')
        self.mode = self.p.MetastableFreqSweepExp.Mode
        self.prep = self.p.PSMetastableFreqSweep.prep_state
    ...
    def run(self, cxn, context):
        ...
        freq = np.linspace(self.start_frequency['MHz'],self.stop_frequency['MHz'],\
                    int((abs(self.stop_frequency['MHz']-\
                    self.start_frequency['MHz'])/self.step_frequency['MHz']) +1))
        ...
        for i in range(len(freq)):
            ...
            while True:
                ...
                self.dv.add(np.column_stack((freq[i], fid_sd)), context = self.c_prob)
    ...
    def set_up_datavault(self):
        ...
        dataset = self.dv.new('metastable_freq_prob',[('num', 'MHz')], \
        ...
        dataset2 = self.dv.new('metastable_freq_hist',[('run', 'arb u')],\
                               [('Counts', 'DC_Hist', 'num'), \
...
if __name__ == '__main__':
    ...
    exprt = metastable_freq_sweep(cxn = cxn)
