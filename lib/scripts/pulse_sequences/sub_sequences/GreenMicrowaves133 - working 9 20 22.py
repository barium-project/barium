...
class green_microwaves_133(pulse_sequence):
    ...
    def sequence(self):
        ...
        if p.microwave_duration != 0:
            ...
            if p.USE_532 == 'True':
                self.addDDS(p.channel_532, self.start + switch_on_delay - amp_change_delay, p.microwave_duration, p.frequency_532, p.amplitude_532)
                self.addTTL(p.TTL_532_AOM, self.start + switch_on_delay - amp_change_delay,  p.microwave_duration)           
