...
from barium.lib.scripts.pulse_sequences.E2LaserSweepXRSB133 import e2_laser_sweep_xrsb_133 as main_sequence
...
class optical_rabi_flopping_xrsb(experiment):

    name = 'Optical Rabi Flopping XRSB'

    exp_parameters = [
                      ('OpticalRabiFloppingXRSB', 'Sequences_Per_Point'),
                      ('OpticalRabiFloppingXRSB', 'Start_Time'),
                      ('OpticalRabiFloppingXRSB', 'Stop_Time'),
                      ('OpticalRabiFloppingXRSB', 'Time_Step'),
                      ('OpticalRabiFloppingXRSB', 'dc_threshold'),
                      ('OpticalRabiFloppingXRSB', 'HP_Amplitude_spectroscopy'),
                      ('OpticalRabiFloppingXRSB', 'HP_Amplitude_rsb'),
                      ('OpticalRabiFloppingXRSB', 'Frequency'),
                      ('OpticalRabiFloppingXRSB', 'Mode'),
                      ('OpticalRabiFloppingXRSB', 'exps_to_average'),
                      ('OpticalRabiFloppingXRSB', 'RSB_Frequency')
                      ]
    ...
    def initialize(self, cxn, context, ident):
        self.ident = ident
        self.cxn = labrad.connect(name = 'Optical Rabi Flopping XRSB')
        self.cxnwlm = labrad.connect(multiplexer_config.ip, name = 'Optical Rabi Flopping XRSB', password = 'lab')
...
#        self.HPA = self.cxn.hp8672a_server
        ...
        self.HP3_rsb = self.cxn.hp8673server2
        ...
        self.cycles = self.p.OpticalRabiFloppingXRSB.Sequences_Per_Point
        self.averages = int(self.p.OpticalRabiFloppingXRSB.exps_to_average)
        self.start_time = self.p.OpticalRabiFloppingXRSB.Start_Time
        self.stop_time = self.p.OpticalRabiFloppingXRSB.Stop_Time
        self.time_step = self.p.OpticalRabiFloppingXRSB.Time_Step
        self.state_detection = self.p.OpticalRabiFloppingXRSB.State_Detection
        self.dc_thresh = self.p.OpticalRabiFloppingXRSB.dc_threshold
        self.hp_amp = self.p.OpticalRabiFloppingXRSB.HP_Amplitude_spectroscopy
        self.hp_amp_rsb = self.p.OpticalRabiFloppingXRSB.HP_Amplitude_rsb
        self.freq = self.p.OpticalRabiFloppingXRSB.Frequency
        self.disc = self.pv.get_parameter('StateReadout','state_readout_threshold')
        self.mode = self.p.OpticalRabiFloppingXRSB.Mode
        self.rsb_frequency = self.p.OpticalRabiFloppingXRSB.RSB_Frequency
...
##### We're using a different GPIB source from the microwaves (uwave_sweep code copied to make this experiment)
        ...
        self.HP3_rsb.select_device(self.device_mapC['GPIB0::9'])
    ...
    def run(self, cxn, context):
        ...
        self.set_hp_frequency(self.freq['MHz'])
        self.set_rsb_hp_frequency(self.rsb_frequency['MHz'])
        time.sleep(.3) # time to switch frequencies
        
        self.set_hp_amplitude(self.hp_amp['dBm'])
        self.set_rsb_hp_amplitude(self.hp_amp_rsb['dBm'])
        time.sleep(.3) # time to switch amplitude

        self.set_hp_rf_state(True)
        self.set_rsb_hp_rf_state(True)
...
#        loop_iter = 0        
    ...
    def set_up_datavault(self):
        ...
        dataset = self.dv.new('OpticalRabiFloppingXRSB_prob',[('t', 'MHz')], [('Probability', 'Probability', 'num')], context = self.c_prob)
        # add dv params
...
#        self.dv.add_parameter('1762 Frequency', self.bristol.get_frequency(), context = self.c_prob)
        ...
        dataset1 = self.dv.new('OpticalRabiFloppingXRSB',[('t', 'MHz')], [('Counts', 'Counts', 'num')], context = self.c_hist)
        # add dv params
        for parameter in self.p:
        ...
        dataset2 = self.dv.new('OpticalRabiFloppingXRSB_dc_hist',[('run', 'arb u')],\
...
##                    break
    ...
    def get_device_map(self):
        ...
        gpib_listB = FrequencyControl_config.gpibB
        gpib_listC = FrequencyControl_config.gpibC
        gpib_listD = FrequencyControl_config.gpibC
        ...
        devices = self.HP3_rsb.list_devices()
        print(devices)
        for i in range(len(gpib_listD)):
            for j in range(len(devices)):
                if devices[j][1].find(gpib_listD[i]) > 0:
                    self.device_mapC[gpib_listD[i]] = devices[j][0]
                    break
    ...
    def set_rsb_hp_rf_state(self, state):
        self.HP3_rsb.rf_state(state)
    ...
    def set_rsb_hp_amplitude(self, amp):
        self.HP3_rsb.set_amplitude(WithUnit(amp,'dBm'))                
##    def set_hp_frequency(self, freq):
##        self.HP3.set_frequency(WithUnit(freq,'MHz'))
##         #self.HP3.set_frequency(WithUnit(int(freq),'MHz'))
    def set_rsb_hp_frequency(self, freq):
        self.HP3_rsb.set_frequency(WithUnit(freq,'MHz'))     
...
if __name__ == '__main__':
    ...
    exprt = optical_rabi_flopping_xrsb(cxn = cxn)
